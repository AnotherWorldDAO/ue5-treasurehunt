...
async function main() {
  ...
  console.log(
    `MockERC20 Contract deployed to ${MockERC20.address} on ${network.name}`
  );
  ...
  console.log(
    `GamePrize Contract deployed to ${GamePrize.address} on ${network.name}`
  );
  ...
  tx = await MockERC20.connect(deployer).transfer(
    GamePrize.address,
