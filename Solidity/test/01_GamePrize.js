...
describe("Game Prize contract", function () {
  async function deployTokenFixture() {
    ...
    const GamePrizeContract = await ethers.getContractFactory(
      "GamePrize"
    );
    ...
    await MockERC20.transfer(GamePrize.address, ethers.utils.parseEther("1000.0"));

    await GamePrize.connect(owner).setAmount(MockERC20.address, ethers.utils.parseEther("1.0"));
  ...
  describe("Deployment", function () {
    ...
    it("Should have a correct GamePrize balance", async function () {
      ...
      expect(await MockERC20.balanceOf(GamePrize.address)).to.equal(ethers.utils.parseEther("1000.0"));
    });
  });
  ...
  describe("Airdrop", function () {
    ...
    it("Should drip correctly", async function () {
      const { MockERC20, GamePrize, owner, operator, player1, player2 } = await loadFixture(
        deployTokenFixture
      );

      await GamePrize.connect(operator).drip(MockERC20.address, player1.address);
      await GamePrize.connect(operator).drip(MockERC20.address, player2.address);

      await expect(GamePrize.connect(operator).drip(MockERC20.address, player1.address))
        .to.emit(GamePrize, "Drip")
        .withArgs(MockERC20.address, player1.address, ethers.utils.parseEther("1.0"));
      
      await expect(await MockERC20.balanceOf(player1.address)).to.equal(ethers.utils.parseEther("2.0"));

      await expect(await MockERC20.balanceOf(player2.address)).to.equal(ethers.utils.parseEther("1.0"));
  ...
  describe("Withdraw", function () {
    ...
    it("Should drip and withdraw correctly", async function () {
      ...
      await expect(GamePrize.connect(owner).withdrawErc20(MockERC20.address))
        ...
        .withArgs(owner.address, MockERC20.address, ethers.utils.parseEther("999.0"));
      
      await expect(await MockERC20.balanceOf(owner.address)).to.equal(await MockERC20.totalSupply());

      await expect(await MockERC20.balanceOf(MockERC20.address)).to.equal(ethers.utils.parseEther("0.0"));
